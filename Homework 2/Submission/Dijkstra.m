function [found_goal, path, cost_to_goal] = Dijkstra(graph, start_node, goal_node, print_msg)
    if print_msg
        disp("Finding path with Dijkstra...");
    end
    found_goal = false;
    cost_to_goal = Inf;
    ...
    O = Inf(O_size, 3);   % Creation of set O 
        ...
        if C(u, 1) == 0        % Check if u is in C
            ...
            for n = N            % All neighbors of u...
                if C(n, 1) == 0     % ... that are not in C ...
                    ...
                    O(O_back,:) = [n, u, u_cost + cost_u_n]; % ... are added to O with u as their backpointer and cost
                    O = sortrows(O, 3);
